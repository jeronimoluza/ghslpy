...
def vectorize(data: xr.Dataset):
    ...
    if "time" in data.sizes and data.sizes["time"] > 1:
        ...
        for time_val in data.time.values:
            ...
            gdf = geocube_vectorize(
                time_slice.to_array().rename(var_name).squeeze().astype(float)
            )
...
def _apply_smod_classification(gdf):
    ...
    gdf["GHS_SMOD"] = gdf["GHS_SMOD"].map(
        lambda x: class_mapping.get(int(x), f"Unknown class {int(x)}"),
        na_action="ignore",
    )

    # Add domain column (Urban domain or Rural domain)
    domain_mapping = smod_info.get("domains", {})
    gdf["domain"] = gdf["class_value"].map(
        lambda x: domain_mapping.get(int(x), "Unknown domain"),
        na_action="ignore",
    )
...
def _apply_built_c_classification(gdf):
    ...
    gdf["GHS_BUILT_C"] = gdf["GHS_BUILT_C"].map(
        lambda x: class_mapping.get(int(x), f"Unknown class {int(x)}"),
        na_action="ignore",
    )
